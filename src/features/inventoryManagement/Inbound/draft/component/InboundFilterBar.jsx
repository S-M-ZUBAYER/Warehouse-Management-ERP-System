...
// ─────────────────────────────────────────────────────────────────────────────
// InboundFilterBar — shared by Draft (Image 1+2) and OnTheWay (Image 5+6)
// Fields: Select Warehouse | Created Time type | All time filter | Date range | Inbound No. | Search | Search btn
// Image 2 shows: Created Time dropdown (Created Time / Estimated arrival time)
//                All dropdown (All/Yesterday/Today/Last 7 days/Last 30 days)
//                Inbound No. dropdown (Inbound No./SKU Name/GTIN)
// ─────────────────────────────────────────────────────────────────────────────
...
export default function InboundFilterBar({
  ...
  const DropBtn = ({ value, onClick, open, className = "" }) => (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      <ChevronDown
        size={13}
        className={`text-slate-400 ml-2 flex-shrink-0 transition-transform ${open ? "rotate-180" : ""}`}
      />
    </button>
  );
    ...
    <div className="absolute left-0 top-full mt-1 z-30 bg-white rounded-xl border border-surface-border shadow-lg py-1 min-w-full w-max">
      {options.map((opt) => (
        <button
          ...
          className={`w-full text-left px-4 py-2 text-sm transition-colors ${selected === opt ? "text-primary font-semibold bg-blue-50" : "text-slate-700 hover:bg-surface-card"}`}
        >
          {opt}
        </button>
      ))}
  ...
  return (
    <div className="bg-white rounded-xl border border-surface-border p-4 font-body">
      <div className="flex items-end gap-2.5 flex-wrap">
        ...
        {/* Select Time type — Created Time / Estimated arrival time */}
        ...
        <div className="w-32 relative mt-5" ref={inboundRef}>
          <DropBtn
            ...
            open={showInboundDrop}
            className="w-full"
          />
          {showInboundDrop && (
            <DropMenu
              options={INBOUND_TYPES}
        ...
        {/* Search button */}
        <button className="mt-5 px-5 py-2 text-sm font-semibold bg-primary hover:bg-primary-dark text-white rounded-lg transition-colors whitespace-nowrap">
          Search
        </button>
      </div>
    </div>
