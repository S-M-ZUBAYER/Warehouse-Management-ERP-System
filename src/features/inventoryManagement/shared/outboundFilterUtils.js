...
export const getOutboundSearchField = (outboundType) =>
  SEARCH_FIELDS[outboundType] ?? SEARCH_FIELDS["Outbound No."];

export const getOutboundDateField = (timeType) =>
...
export const formatOutboundDateRange = ({ dateFrom, dateTo }) => {
...
const lineMatches = (line, query, outboundType) => {
  const sku = line?.merchantSku ?? {};

  if (outboundType === "GTIN") {
...
export const filterOutboundItems = (
  items,
  { warehouseId, search, outboundType, timeType, dateFrom, dateTo },
) => {
  ...
  return items.filter((item) => {
    ...
    if (query) {
      const matchesSearchType = outboundType === "Outbound No."
        ? (
            valueMatches(item.outbound_id, query) ||
            valueMatches(item.tracking_number, query) ||
            valueMatches(item.supplier_reference, query)
          )
        : item.lines?.some((line) => lineMatches(line, query, outboundType));
