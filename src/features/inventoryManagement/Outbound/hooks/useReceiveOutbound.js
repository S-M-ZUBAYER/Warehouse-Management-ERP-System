...
const fetchOutboundDetail = (id) =>
    api.get(`/outbound/${id}`).then((r) => r.data);

const receiveOutbound = ({ id, body }) =>
    api.put(`/outbound/${id}/receive`, body).then((r) => r.data);

const receiveOutboundTargets = async ({ target, body }) => {
    const rows = Array.isArray(target) ? target : [target];

    if (rows.length > 1) {
        const details = Array.isArray(body?.details) ? body.details : [];
        const results = await Promise.all(rows.map(async (row) => {
            const detail = details.find((item) => String(item?.id) === String(row.id)) ?? await fetchOutboundDetail(row.id);
            const lines = detail?.lines ?? [];

            if (!lines.length) {
                throw new Error(`No lines found for ${row.outbound_id ?? row.id}`);
            }

            return receiveOutbound({
                id: row.id,
                body: {
    ...
    const result = await receiveOutbound({ id: rows[0].id, body });
    return { results: [result], count: 1 };
};

export function useReceiveOutbound({ onSuccess } = {}) {
    ...
    const { data: outboundDetail, isLoading: detailLoading } = useQuery({
        queryKey: OUTBOUND_KEYS.detail(receiveTarget?.id),
        queryFn: () => fetchOutboundDetail(receiveTarget.id),
        enabled: !isBulkReceive && !!receiveTarget?.id,
        staleTime: 0,
    });

    const lines = outboundDetail?.lines ?? [];
    const bulkDetailQueries = useQueries({
        queries: isBulkReceive
            ? receiveTarget.map((target) => ({
                queryKey: OUTBOUND_KEYS.detail(target.id),
                queryFn: () => fetchOutboundDetail(target.id),
                enabled: !!target?.id,
                staleTime: 0,
            }))
            : [],
    ...
    const receiveMutation = useMutation({
        mutationFn: receiveOutboundTargets,
        onSuccess: ({ results = [], count = 0 }) => {
            if (count > 1) {
                toast.success(`${count} outbound orders received`);
            } else {
                toast.success(`Outbound ${results[0]?.outbound_id ?? ''} received`);
            }
        ...
        onError: (err) => {
            toast.error(err?.response?.data?.message ?? err?.message ?? 'Failed to receive outbound');
        },
    });
    ...
    const confirmReceive = useCallback(() => {
        ...
        if (Array.isArray(receiveTarget) && receiveTarget.length > 1) {
            ...
            if (bulkDetails.some((detail) => !detail?.lines?.length)) {
                toast.error('No lines found for one or more selected outbound orders');
                return;
            }
        ...
        if (!lines.length) {
            toast.error('No lines found for this outbound order');
            return;
        }
        if (!canConfirmReceive) {
            toast.error('Please enter received quantity for every line');
