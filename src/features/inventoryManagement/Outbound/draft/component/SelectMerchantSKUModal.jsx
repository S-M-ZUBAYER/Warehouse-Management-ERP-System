...
// All state and API calls come from the parent via useCreateOutbound hook
// ─────────────────────────────────────────────────────────────────────────────
                  ...
                  <tbody className="divide-y divide-surface-border">
                    {pickerSkus.length === 0 ? (
                      ...
                      pickerSkus.map((sku) => {
                        ...
                        const available = sku.qty_available ?? sku.qty_on_hand ?? 0;
                        return (
                          <tr
                            key={sku.id}
                            onClick={() => available > 0 && toggleSku(sku.id)}
                            className={`transition-colors hover:bg-slate-50 ${available > 0 ? "cursor-pointer" : "cursor-not-allowed opacity-60"} ${isChecked ? "bg-blue-50/60" : ""}`}
                          >
                    ...
                    {pickerPreviewItems.map((sku) => {
                      ...
                      <tr key={sku.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-2 pl-4">
                          ...
                          />
                          <p className="mt-0.5 text-[10px] text-slate-400">Max {available}</p>
                        </td>
                        <td className="py-2 pl-4">
                          <button
