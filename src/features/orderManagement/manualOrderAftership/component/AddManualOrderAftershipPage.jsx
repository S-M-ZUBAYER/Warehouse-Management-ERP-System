...
import {
  createManualOrder,
  fetchAfterShipConfig,
  fetchAfterShipRates,
  fetchAfterShipShipperAccounts,
...
const normalizeCountryCode = (value, fallback = "PH") => {
  ...
  if (["PHILIPPINES", "PHL"].includes(raw)) return "PH";
  if (["VIETNAM", "VNM"].includes(raw)) return "VN";
  if (["THAILAND", "THA"].includes(raw)) return "TH";
...
export default function AddManualOrderAftershipPage({ mode = "order", onBack, onCreated }) {
  ...
  const [afterShipContent, setAfterShipContent] = useState("Product");
  const [shipperAccountId, setShipperAccountId] = useState("");
  ...
  useEffect(() => {
    ...
    setSenderForm((prev) => ({
      ...prev,
      senderName: prev.senderName || selectedWarehouse.manager_name || selectedWarehouse.name || "",
      company: prev.company || selectedWarehouse.name || "",
      phone: prev.phone || selectedWarehouse.phone || "",
      address: prev.address || selectedWarehouse.location || "",
      city: prev.city || selectedWarehouse.city || "",
      country: countryOptions.some((country) => country.value === normalizeCountryCode(selectedWarehouse.country, "PH"))
        ? normalizeCountryCode(selectedWarehouse.country, "PH")
        : prev.country || "PH",
    }));
  }, [selectedWarehouse]);

  useEffect(() => {
    if (addedProducts.length && afterShipContent === "Product") {
      const firstNames = addedProducts.map((product) => product.name || product.sku).filter(Boolean).slice(0, 2);
      if (firstNames.length) setAfterShipContent(firstNames.join(", ").slice(0, 35));
    }
  }, [addedProducts, afterShipContent]);
  ...
  const afterShipCountry = normalizeCountryCode(senderForm.country, "PH");

  const afterShipConfigQuery = useQuery({
    queryKey: ["manual-order-aftership-config", afterShipCountry],
    queryFn: () => fetchAfterShipConfig(afterShipCountry),
    enabled: Boolean(afterShipCountry),
    staleTime: 1000 * 60,
  });

  const shipperAccountsQuery = useQuery({
    queryKey: ["manual-order-aftership-shipper-accounts", afterShipCountry],
    queryFn: () => fetchAfterShipShipperAccounts({ country: afterShipCountry }),
    enabled: Boolean(afterShipCountry && afterShipConfigQuery.data?.apiKeySet !== false),
    staleTime: 1000 * 60,
  });

  const shipperAccountsRaw = Array.isArray(shipperAccountsQuery.data)
    ? shipperAccountsQuery.data
    : shipperAccountsQuery.data?.shipperAccounts || shipperAccountsQuery.data?.accounts || [];

  const shipperAccountOptions = shipperAccountsRaw.map((account) => ({
    value: account.id || account.shipperAccountId || account.accountId || account.slug,
    label: account.name || account.accountName || account.slug || account.id || account.shipperAccountId,
  })).filter((account) => account.value);

  const afterShipQuery = useQuery({
    queryKey: [
      "manual-order-aftership-rates",
    ...
    queryFn: () =>
      fetchAfterShipRates({
        ...
        sendCountry: buyerForm.country,
        postcode: buyerForm.zipCode,
        city: buyerForm.city,
    ...
    enabled: Boolean(
      orderForm.warehouseId &&
        ...
        senderForm.country &&
        buyerForm.country &&
        afterShipConfigQuery.data?.apiKeySet !== false
    ),
    staleTime: 1000 * 60,
  });

  const afterShipServices = afterShipQuery.data?.services || [];
  const selectableAfterShipServices = paymentType === "COD"
    ? afterShipServices.filter((service) => service.codAvailable)
    : afterShipServices;
  const logisticOptions = selectableAfterShipServices.map((service) => ({
  ...
  const selectedLogistic = afterShipServices.find(
  ...
  const handleSenderChange = (e) => {
    const { name, value } = e.target;
    const nextValue = name === "country" ? normalizeCountryCode(value, "PH") : value;
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ payload }) => {
      const data = await createManualOrder(payload);
      const savedOrder = data?.order || data?.manualOrder || data;
      const normalizedOrder = normalizeManualOrder({
        ...(savedOrder || {}),
        afterShip: data?.afterShip || savedOrder?.afterShip,
        pdfUrl: data?.pdfUrl || data?.waybillPdfUrl || data?.awbLink || data?.afterShip?.labelUrl,
        pdfFilename: data?.pdfFilename || data?.fileName || data?.filename,
      });
      return { data, normalizedOrder };
    },
    onSuccess: ({ data, normalizedOrder }) => {
      if (data?.afterShipError) {
        toast.error(data?.message || data.afterShipError);
      } else if (data?.afterShip?.trackingNumber) {
        toast.success(`AfterShip label created: ${data.afterShip.trackingNumber}`);
      } else {
  ...
  const updateQty = (id, val) =>
    setAddedProducts((prev) =>
      prev.map((item) =>
        item.id === id
          ? { ...item, qty: clampQuantity(val, item.availableForPlatform ?? item.available) }
          : item
      )
    );

  const validateAfterShipBeforeBooking = () => {
    const missing = [];
    const senderCountry = normalizeCountryCode(senderForm.country, "PH");
    const receiverCountry = normalizeCountryCode(buyerForm.country, "PH");
    if (!selectedLogistic) missing.push("AfterShip courier service");
    ...
    if (!senderForm.city.trim()) missing.push("sender city");
    if (!senderForm.state.trim()) missing.push("sender state");
    ...
    if (!buyerForm.city.trim()) missing.push("receiver city");
    if (!buyerForm.state.trim()) missing.push("receiver state");

    if (!countryOptions.some((country) => country.value === senderCountry) || !countryOptions.some((country) => country.value === receiverCountry)) {
      toast.error("AfterShip manual booking is enabled only for PH, VN, TH, ID, MY, and SG.");
      return false;
    }
    if (senderCountry !== receiverCountry) {
      toast.error("AfterShip manual booking supports domestic shipment only. Sender and receiver country must be the same.");
      return false;
    }
    if (paymentType === "COD" && selectedLogistic && !selectedLogistic.codAvailable) {
      toast.error("Selected AfterShip courier does not support COD. Select a COD-supported courier or change payment type to Prepaid.");
      return false;
    }
    ...
    if (missing.length) {
      toast.error(`Missing AfterShip information: ${missing.join(", ")}`);
      return false;
    }
    return true;
  };
  ...
  const buildManualOrderPayload = ({ bookNow = false } = {}) => ({
    ...
    logisticServiceId: selectedLogistic?.serviceType || selectedLogistic?.serviceId || orderForm.logistic,
    ...
    logisticRaw: {
      ...
      afterShip: {
        selectedRate: selectedLogistic || null,
        shipperAccountId,
        country: afterShipCountry,
      },
    },
    currency: afterShipConfigQuery.data?.currency || selectedLogistic?.currency || orderForm.currency || "USD",
    buyer: buyerForm,
  ...
  const handleSave = ({ bookNow = false } = {}) => {
    ...
    if ((bookNow || orderForm.logistic) && !validateAfterShipBeforeBooking()) return;

    saveMutation.mutate({ payload: buildManualOrderPayload({ bookNow }) });
  };
    ...
    <div className="space-y-4 font-body">
      ...
      <div className="bg-white rounded-xl border border-surface-border p-4">
        <div className="grid grid-cols-7 gap-3">
          <SelectField
            ...
            options={[
              { value: "easyparcel", label: "EasyParcel" },
              { value: "aftership", label: "AfterShip" },
            ]}
            value="aftership"
            onChange={(event) => {
              if (event.target.value === "easyparcel") {
                window.location.href = "/warehouse_management/orders/manual_order";
              }
            }}
          />
          <SelectField
          ...
          <SelectField
            ...
            name="logistic"
            placeholder={
              afterShipQuery.isFetching
                ? "Loading AfterShip..."
                : afterShipConfigQuery.data?.apiKeySet === false
                  ? "AfterShip not configured"
        ...
        {afterShipConfigQuery.data?.apiKeySet === false && (
          <p className="mt-2 text-xs text-amber-600">
            AfterShip API key is not configured for this country. Ask an admin to configure {`AFTERSHIP_${afterShipCountry}_API_KEY`} or global AFTERSHIP_API_KEY.
          </p>
        )}
        {afterShipQuery.data?.message && (
          <p className="mt-2 text-xs text-amber-600">{afterShipQuery.data.message}</p>
        )}
      </div>
        ...
        <div className="col-span-2 space-y-4">
          <div className="bg-white rounded-xl border border-surface-border p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-slate-800 font-display">Sender / Pickup Information</h3>
              <span className="text-[11px] text-slate-400">Used only for AfterShip rates and booking</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              <FormInput className="col-span-2" label="Pickup Address" placeholder="Full pickup address" name="address" value={senderForm.address} onChange={handleSenderChange} />
            ...
            <div className="grid grid-cols-3 gap-3">
              <FormInput label="State" placeholder="State / province" name="state" value={senderForm.state} onChange={handleSenderChange} />
          ...
          >
            ...
            <div className="grid grid-cols-5 gap-3">
              <FormInput label="State" placeholder="State / province" name="state" value={buyerForm.state} onChange={handleBuyerChange} />
          ...
          <div className="bg-white rounded-xl border border-surface-border p-5 min-h-[360px]">
            <div className="relative flex items-center gap-3 mb-4">
              ...
              {searchResults && (
                <div className="absolute left-0 top-[calc(100%+8px)] z-20 w-full overflow-hidden rounded-lg border border-surface-border bg-white shadow-lg">
          ...
          <div className="bg-white rounded-xl border border-surface-border p-4">
            ...
            <div className="grid grid-cols-3 gap-3 mt-3">
              <FormInput className="col-span-2" label="AfterShip Content" placeholder="Product description for AfterShip shipment" name="afterShipContent" value={afterShipContent} onChange={(e) => setAfterShipContent(e.target.value)} />
              <SelectField
                label="Shipper Account"
                name="shipperAccountId"
                placeholder={shipperAccountsQuery.isFetching ? "Loading accounts..." : "Optional account"}
                options={shipperAccountOptions}
                value={shipperAccountId}
                onChange={(e) => setShipperAccountId(e.target.value)}
              />
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-surface-border p-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-sm font-bold text-slate-800 font-display">AfterShip Courier Rates</h3>
              ...
              <button
                onClick={() => afterShipQuery.refetch()}
                disabled={!orderForm.warehouseId || !senderForm.postcode || !buyerForm.zipCode || afterShipQuery.isFetching || afterShipConfigQuery.data?.apiKeySet === false}
                className="px-4 py-2 text-xs font-semibold border border-surface-border rounded-lg text-slate-600 hover:bg-surface-card disabled:opacity-60"
              >
                {afterShipQuery.isFetching ? "Checking..." : "Check Rates"}
              </button>
            </div>

            {afterShipServices.length === 0 ? (
              <div className="rounded-lg border border-dashed border-surface-border p-4 text-xs text-slate-400">
                {afterShipQuery.data?.configured === false
                  ? afterShipQuery.data?.message || "AfterShip is not configured for this country."
                  : afterShipQuery.data?.message || "No courier rates loaded yet."}
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {afterShipServices.map((service) => {
            ...
            <p className="mt-4 text-xs text-slate-500">
              Save + Submit AfterShip creates the manual order and requests the AfterShip label in the same save.
            </p>
          </div>
        </div>
      ...
      <div className="flex justify-end gap-3 pt-1">
        ...
        <button
          ...
          className="px-7 py-2.5 text-sm font-semibold border border-primary text-primary rounded-xl hover:bg-primary/5 transition-colors disabled:opacity-60"
        >
        ...
        >
          {saveMutation.isPending ? "Processing..." : "Save + Submit AfterShip"}
        </button>
      </div>
    </div>
  );
