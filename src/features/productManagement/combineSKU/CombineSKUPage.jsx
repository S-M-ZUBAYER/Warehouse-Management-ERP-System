    ...
    <div className="space-y-4 font-body">
      ...
      <div className="bg-white rounded-xl border border-surface-border overflow-hidden">
        ...
        <div className="flex justify-end gap-3 px-5 py-4 border-t border-surface-border">
          <button onClick={() => exportRowsToCsv(bundles.filter((bundle) => selectedIds.includes(bundle.id)), [
          ...
          ], "combine-skus.csv", "combine SKU")} className="flex items-center gap-2 px-14 py-2 text-base font-semibold border border-surface-border rounded-lg text-slate-700 bg-white hover:bg-surface-card transition-colors">
      ...
      <RecordDetailModal
        open={!!detailBundle}
        title="Combine SKU Details"
        subtitle={detailBundle?.combine_sku_code}
        record={detailBundle}
        onClose={() => setDetailBundle(null)}
        fields={[
          { label: "Combine SKU", key: "combine_sku_code" },
          { label: "Bundle Name", key: "combine_name" },
          { label: "Computed Stock", key: "computed_quantity" },
          { label: "Item Count", render: (row) => row.items?.length || 0 },
